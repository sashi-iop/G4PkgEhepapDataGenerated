...
void plot_results(
    const char* simFile  = "test_run10.root",
    const char* digiFile = ""
) {
    ...
    c1->cd(1);
    if (h_ecal) { h_ecal->SetLineColor(kBlue+1); h_ecal->SetLineWidth(2); h_ecal->Draw(); }
    else cout << "  [SKIP] ecalenergy not found" << endl;

    c1->cd(2);
    if (h_ecal2d) { h_ecal2d->Draw("COLZ"); }
    else cout << "  [SKIP] h2decalenergy not found" << endl;

    c1->cd(3);
    if (h_posRE) { h_posRE->SetLineColor(kGreen+2); h_posRE->SetLineWidth(2); h_posRE->Draw(); }
    else cout << "  [SKIP] pPosRE not found" << endl;

    c1->cd(4);
    if (h_posEE) { h_posEE->SetLineColor(kRed+1); h_posEE->SetLineWidth(2); h_posEE->Draw(); }
    else cout << "  [SKIP] pPosEE not found" << endl;
    ...
    if (nhcalFound == 0) nhcalFound = 1;

    int nx = (int)ceil(sqrt((double)nhcalFound));
    int ny = (int)ceil((double)nhcalFound / nx);
    c2->Divide(nx, ny);

    int colors[] = {kBlue+1, kRed+1, kGreen+2, kMagenta+1, kOrange+1,
                    kCyan+1, kViolet+1, kTeal+1, kPink+1, kSpring+2,
                    kAzure+1, kYellow+2, kGray+1, kBlue-4, kRed-4,
                    kGreen-4, kMagenta-4};
    ...
    c3->cd(1);
    if (h_posRH) { h_posRH->SetLineColor(kBlue+1); h_posRH->SetLineWidth(2); h_posRH->Draw(); }

    c3->cd(2);
    if (h_posEH) { h_posEH->SetLineColor(kRed+1); h_posEH->SetLineWidth(2); h_posEH->Draw(); }

    c3->cd(3);
    if (h_posPH) { h_posPH->SetLineColor(kGreen+2); h_posPH->SetLineWidth(2); h_posPH->Draw(); }
    ...
    if (ntrkFound > 0) {
        ...
        for (int i = 0; i < ntrkFound; i++) {
            c4->cd(i + 1);
            TH1F *h = (TH1F*)fSim->Get(Form("trkenergy_L%d", i));
            if (h) { h->SetLineColor(colors[i % 17]); h->SetLineWidth(2); h->Draw(); }
        }
    }
    ...
    if (T1) {
        ...
        if (hm) { hm->SetTitle("Generated Momentum;Momentum (GeV);Events"); hm->SetLineWidth(2); }

        c6->cd(2);
        T1->Draw("nsimhtHL>>h_nhits(100, 0, 500)", "", "");
        TH1F *hn = (TH1F*)gDirectory->Get("h_nhits");
        if (hn) { hn->SetTitle("HCAL Hits per Event;N hits;Events"); hn->SetLineWidth(2); }

        c6->cd(3);
        T1->Draw("nsimhtEC>>h_necal(100, 0, 500)", "", "");
        TH1F *hne = (TH1F*)gDirectory->Get("h_necal");
        if (hne) { hne->SetTitle("ECAL Hits per Event;N hits;Events"); hne->SetLineWidth(2); }

        c6->cd(4);
        T1->Draw("pidin[0]>>h_pid(500, -250, 250)", "", "");
        TH1F *hp = (TH1F*)gDirectory->Get("h_pid");
        if (hp) { hp->SetTitle("Particle ID;PID;Events"); hp->SetLineWidth(2); }
        ...
        if (hpe) {
            ...
            hpe->SetLineColor(kRed+1); hpe->SetLineWidth(2);
        }

        c7->cd(2);
        // Extract depth from bits 0-2 of cell ID (which is bits 17-19 of packed word)
        ...
        if (hd) {
            ...
            hd->SetLineColor(kGreen+2); hd->SetLineWidth(2);
        }
    ...
    if (digiStr.Length() == 0) {
        cout << "\nNo digitisation file found. Skipping digitisation plots." << endl;
    ...
    if (hts) {
        ...
        hts->SetLineColor(kBlue+1); hts->SetLineWidth(2);
        if (hts->GetEntries() > 10) hts->Fit("gaus", "Q");
    }
    ...
    if (htd) {
        ...
        htd->SetLineColor(kRed+1); htd->SetLineWidth(2);
        if (htd->GetEntries() > 10) htd->Fit("gaus", "Q");
    }
    ...
    if (hhs) {
        ...
        hhs->SetLineColor(kGreen+2); hhs->SetLineWidth(2);
        if (hhs->GetEntries() > 10) hhs->Fit("gaus", "Q");
    }
    ...
    if (hhd) {
        ...
        hhd->SetLineColor(kOrange+1); hhd->SetLineWidth(2);
        if (hhd->GetEntries() > 10) hhd->Fit("gaus", "Q");
    }
    ...
    if (hovs) { hovs->SetLineColor(kBlue+1); hovs->SetLineWidth(2);
                hovs->SetTitle("ECAL+HCAL: Simulated vs Digitised;Energy (GeV);Events"); }
    if (hovd) { hovd->SetLineColor(kRed+1); hovd->SetLineWidth(2); hovd->SetLineStyle(2); }

    TLegend *leg1 = new TLegend(0.6, 0.7, 0.88, 0.85);
    if (hovs) leg1->AddEntry(hovs, "Simulated", "l");
    if (hovd) leg1->AddEntry(hovd, "Digitised", "l");
    leg1->Draw();
    ...
    if (hovhs) { hovhs->SetLineColor(kGreen+2); hovhs->SetLineWidth(2);
                 hovhs->SetTitle("HCAL-only: Simulated vs Digitised;Energy (GeV);Events"); }
    if (hovhd) { hovhd->SetLineColor(kOrange+1); hovhd->SetLineWidth(2); hovhd->SetLineStyle(2); }

    TLegend *leg2 = new TLegend(0.6, 0.7, 0.88, 0.85);
    if (hovhs) leg2->AddEntry(hovhs, "Simulated", "l");
    if (hovhd) leg2->AddEntry(hovhd, "Digitised", "l");
    leg2->Draw();
    ...
    c10->cd(1);
    T2->Draw("totdigienr/totsimenr>>h_ratio_tot(200, 0, 2)",
             "totsimenr>1", "");
    TH1F *hrt = (TH1F*)gDirectory->Get("h_ratio_tot");
    if (hrt) {
        hrt->SetTitle("ECAL+HCAL: Digitised/Simulated;E_{digi}/E_{sim};Events");
        hrt->SetLineColor(kBlue+1); hrt->SetLineWidth(2);
        if (hrt->GetEntries() > 10) hrt->Fit("gaus", "Q");
    }

    c10->cd(2);
    T2->Draw("hcldigienr/hclsimenr>>h_ratio_hcl(200, 0, 2)",
             "hclsimenr>1", "");
    TH1F *hrh = (TH1F*)gDirectory->Get("h_ratio_hcl");
    if (hrh) {
        hrh->SetTitle("HCAL-only: Digitised/Simulated;E_{digi}/E_{sim};Events");
        hrh->SetLineColor(kOrange+1); hrh->SetLineWidth(2);
        if (hrh->GetEntries() > 10) hrh->Fit("gaus", "Q");
    }
    ...
    if (hrt && hrt->GetFunction("gaus")) {
        ...
        cout << "ECAL+HCAL resolution: mean = " << f->GetParameter(1)
             ...
             << ", sigma/mean = " << f->GetParameter(2)/f->GetParameter(1) << endl;
    }
